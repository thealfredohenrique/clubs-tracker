...
function FavoriteCard({ favorite, onRemove, removeTitle }: FavoriteCardProps) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col items-center p-4 bg-slate-900/60 backdrop-blur-sm border border-white/10 rounded-xl transition-all duration-200 hover:border-emerald-500/30 hover:shadow-lg hover:shadow-emerald-500/10"
    >
      {/* Remove Button */}
      <button
        onClick={handleRemove}
        className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center rounded-full bg-slate-900/80 border border-slate-700 opacity-0 group-hover:opacity-100 hover:bg-red-500/20 hover:border-red-500/50 transition-all cursor-pointer"
        title={removeTitle}
      >
        <svg
      ...
      {/* Crest */}
      <div className="w-16 h-16 mb-3 rounded-full bg-slate-800/50 ring-1 ring-white/5 p-1.5 flex items-center justify-center group-hover:scale-105 transition-transform">
        {crestUrl ? (
          <Image
            src={crestUrl}
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-16">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h2 className="text-sm font-medium text-slate-300 uppercase tracking-widest">{t.search.myClubs}</h2>
