    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        if (result.data.length === 0) {
          setError('Nenhum clube encontrado com esse nome.');
        }
      } else {
      ...
      {hasSearched && results.length > 0 && (
        <div className="mt-8">
          <h2 className="text-lg font-semibold text-white mb-4">
            Resultados ({results.length} clube{results.length !== 1 ? 's' : ''})
          </h2>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-400 mt-1">
                        <span>Divisão {club.currentDivision}</span>
      ...
      {hasSearched && results.length === 0 && !error && (
        <div className="mt-6 p-6 bg-gray-800/30 border border-gray-700/50 rounded-xl text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Tente buscar com um nome diferente ou verifique a plataforma.
          </p>
        </div>
      )}
    </div>
